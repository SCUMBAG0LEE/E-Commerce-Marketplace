...
import 'package:bad_tech/screens/product/view_product_page.dart';
...
class _SearchPageState extends State<SearchPage>
  ...
  void searchProducts(String searchText, String selectedCategory, String selectedPrice) {
  List<Product> tempList = [];

  products.forEach((product) {
    if (product.name.toLowerCase().contains(searchText.toLowerCase()) &&
        (selectedCategory.isEmpty || product.category.toLowerCase() == selectedCategory.toLowerCase()) &&
        (selectedPrice.isEmpty || productMatchesPriceRange(product, selectedPrice))) {
...
bool productMatchesPriceRange(Product product, String selectedPrice) {
  if (selectedPrice.isEmpty) {
    return true; // No price filter selected, include all products
  }

  // Split the selectedPrice into minPrice and maxPrice
  final priceRange = selectedPrice.split('-');
  if (priceRange.length == 2) {
    final minPrice = double.tryParse(priceRange[0].replaceAll('\$', '').trim()) ?? 0.0;
    final maxPrice = double.tryParse(priceRange[1].replaceAll('\$', '').trim()) ?? double.infinity;
    
    return product.price >= minPrice && product.price <= maxPrice;
  }

  return true; // Invalid price filter format, include all products
}
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: TextField(
            ...
            onChanged: (value) {
              searchProducts(value, selectedCategory, selectedPrice); // Pass all three arguments
            },
            ...
            decoration: InputDecoration(
              ...
              suffix: TextButton(
                onPressed: () {
                  ...
                  searchProducts('', selectedCategory, selectedPrice);
                },
                child: Text(
                  'Clear',
        ...
        Flexible(
          child: Container(
            ...
            child: searchController.text.isEmpty && selectedCategory.isEmpty && selectedPrice.isEmpty
                ? Center(
              ...
              itemBuilder: (_, index) => Center(
                child: Padding(
                  ...
                  child: InkWell(
                    onTap: () {
                      setState(() {
                        if (selectedCategory == categoryFilter[index]) {
                          selectedCategory = ''; 
                        } else {
                          selectedCategory = categoryFilter[index]; 
                        }
                        searchProducts(searchController.text, selectedCategory, selectedPrice);
                       });
                    },
                    child: Container(
                      padding: EdgeInsets.symmetric(
              ...
              itemBuilder: (_, index) => Center(
                child: Padding(
                  ...
                  child: InkWell(
                    onTap: () {
                      setState(() {
                        if (selectedPrice == priceFilter[index]) {
                          selectedPrice = '';
                        } else {
